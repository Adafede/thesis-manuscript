...
| Software, algorithm| Manubot| [https://manubot.org](https://manubot.org) | QID:[Q96473455](https://www.wikidata.org/wiki/Q96473455) RRID:SCR_018553 | [https://github.com/lotusnprod/lotus-manuscript](https://github.com/lotusnprod/lotus-manuscript)| 
...
| Software, algorithm| Wikidata | [https://www.wikidata.org](https://www.wikidata.org) | QID:[Q2013](https://www.wikidata.org/wiki/Q2013) RRID:SCR_018492| [https://www.wikidata.org/wiki/Wikidata:WikiProject_Chemistry/Natural_products](https://www.wikidata.org/wiki/Wikidata:WikiProject_Chemistry/Natural_products)|
...
This led to the identification of thirty-six electronic [NP](#np) resources as valuable LOTUS input.
Data from the proprietary Dictionary of Natural Products ([DNP](#dnp)) v 29.2) was also used for comparison purposes only and is not publicly disseminated.
[FooDB](https://foodb.ca) was also curated but not publicly disseminated since its license proscribed sharing in Wikidata.
[Appendix 1](#appendix-1-data-sources-list) gives all necessary details regarding electronic [NP](#np) resources access and characteristics.

Manual inspection of each electronic [NP](#np) resource revealed that the structure, organism, and reference fields were widely variable in format and contents, thus requiring standardization to be comparable.
...
For all thirty-eight sources, if a single file or multiple files were accessible *via* a download option including [FTP](#ftp), data was gathered that way.
...
Data extraction scripts for the [DNP](#dnp) are available and should allow users with a [DNP](#dnp) license only to further exploit the data ([src/1_gathering/db/dnp](https://github.com/lotusnprod/lotus-processor/tree/main/src/1_gathering/db/dnp)).
...
For chemical structures, "InChI", "SMILES", and "chemical name" (not necessarily [IUPAC](#iupac)).
...
The generic "external" field was used for all external cross-references to other websites or electronic [NP](#np) resources (e.g. "also in knapsack").
...
Upon electronic [NP](#np) resources harmonization, resulting subcategories were divided and subject to further processing.
...
The structure was reported as [InChI](#inchi), together with its [SMILES](#smiles) and [InChIKey](#inchikey) translation.
The biological organism field was reported as three minimal necessary and sufficient fields, namely its canonical name and the taxon[ID](#id) and taxonomic [DB](#db) corresponding to the latter.
The reference was reported as four minimal fields, namely reference title, [DOI](#doi), [PMCID](#pmcid), and [PMID](#pmid), one being sufficient.
For the forthcoming translation processes, automated solutions were used when available.
However, for specific cases (common or vernacular names of the biological organisms, Traditional Chinese Medicine ([TCM](#tcm)) names, and conversion between digital reference identifiers), no solution existed, thus requiring the use of tailored dictionaries.
...
Allowed structural fields for the sources were divided into two types: structural ([InChI](#inchi), [SMILES](#smiles)) or nominal (chemical name, not necessarily [IUPAC](#iupac)).
If multiple fields were present, structural identifiers were preferred over structure names.
Among structural identifiers, when both identifiers were present, [SMILES](#smiles) was preferred over [InChI](#inchi).
[InChI](#inchi) were translated to [SMILES](#smiles) using the RDKit (2021.09.2) implementation in Python 3.8 ([src/2_curating/2_editing/structure/1_translating/inchi.py](https://github.com/lotusnprod/lotus-processor/tree/main/src/2_curating/2_editing/structure/1_translating/inchi.py)).
They were first converted to [ROMol](https://www.rdkit.org/docs/cppapi/ROMol_8h.html#details) objects which were then converted to [SMILES](#smiles).
When no structural identifier was available, the nominal identifier was translated to [InChI](#inchi) first thanks to [OPSIN](https://github.com/dan2097/opsin) [@doi:10/b2zkr9], a fast Java-based translation open-source solution.
...
In a second step, the FragmentRemover functionality was applied using a list of [SMARTS](#smarts) to detect and remove common counterions and crystallization reagents sometimes occurring in the input [DB](#db).
Finally, the Uncharger function was employed to neutralize molecules when appropriate.

[Molconvert](https://docs.chemaxon.com/display/docs/molconvert.md) function of the MarvinSuite was used for traditional and [IUPAC](#iupac) names translation, Marvin 20.19, [ChemAxon](https://www.chemaxon.com).
...
After manual evaluation, structures remaining as dimers were discarded (all structures containing a "." in their [SMILES](#smiles) were removed).

From the 283,267 initial [InChI](#inchi), 242,068 (85%) sanitized structures were obtained, of which 185,929 (77%) had complete stereochemistry defined.
203,718 (72%) were uploaded to Wikidata.
From the 248,185 initial [SMILES](#smiles), 207,658 (84%) sanitized structures were obtained, of which 98,685 (48%) had complete stereochemistry defined.
...
The original organism strings were treated with [Global Names Finder](https://github.com/gnames/gnfinder) ([GNF](#gnf)) and [Global Names Verifier](https://github.com/gnames/gnverifier) ([GNV](#gnv)), both tools coming from the [Global Names Architecture](http://globalnames.org) ([GNA](#gna)) a system of web services that helps people to register, find, index, check and organize biological scientific names and interconnect on-line information about species.
[GNF](#gnf) allows scientific name recognition within raw text blocks and searches for found scientific names among public taxonomic [DB](#db).
[GNV](#gnv) takes names or lists of names and verifies them against various biodiversity data sources.
Canonical names, their taxon[ID](#id), and the taxonomic [DB](#db) they were found in were retrieved.
When a single entry led to multiple canonical names (accepted synonyms), all of them were kept.
Because both [GNF](#gnf) and [GNV](#gnv) recognize scientific names and not common ones, common names were translated before a second resubmission.
...
For [TCM](#tcm) names, translation dictionaries from TCMID, TMMC, and coming from the Chinese Medicine Board of Australia were aggregated.
...
After this subtraction step, the remaining names were translated from vernacular (common) and [TCM](#tcm) names to scientific names, with help of the dictionaries.
...
The [Rcrossref](https://cran.r-project.org/web/packages/rcrossref) package [@chamberlainRcrossrefClientVarious2020] interfacing with the [Crossref](https://www.crossref.org) [API](#api) was used to translate references from their original subcategory ("original", "publishingDetails", "split", "title") to a [DOI](#doi), the title of its corresponding article, the journal it was published in, its date of publication and the name of the first author.
...
For [DOI](#doi) and [PMID](#pmid), only a single candidate was kept.
All [DOI](#doi)s were also translated with this method, to eventually discard any [DOI](#doi) not leading to an object.
[PMID](#pmid)s were translated, thanks to the entrez_summary function of the [rentrez](https://cran.r-project.org/web/packages/rentrez) package [@doi:10/gh5ptg].
...
An entry was only validated if: *i)* the structure (as any structural descriptor that could be linked to the final sanitized [InChIKey](#inchikey)) was described in the reference *ii)* the producing organism (as any organism descriptor that could be linked to the accepted canonical name) was described in the reference and *iii)* the reference was describing the occurrence of the chemical structure in the biological organism.
...
Except for COCONUT, exceptions to this filter were made for all [DOI](#doi)-based references. 
...
In addition to the diverse search functions available through this web interface, an [API](#api) is also implemented, allowing programmatic [LNPN](#lnpn) querying.
The complete [API](#api) usage is described on the "Documentation" page of the website.
[LNPN](#lnpn) is part of the NaturalProducts.net portal, an initiative aimed at gathering diverse open [NP](#np) resources in one place.
...
It is currently adapted to export directly into the [SSOT](#ssot) thus allowing direct reuse by the processing/curation pipeline.

##### Data Addition 

###### Wikidata

Data is loaded by the Kotlin importer available in the [upload lotus](https://github.com/lotusnprod/lotus-wikidata-interact/blob/main/uploadLotus/README.md) part of [lotus-wikidata-interact](https://github.com/lotusnprod/lotus-wikidata-interact) repository under a [GPL](#gpl) V3 license and imported into Wikidata.
...
The importer is currently being adapted to use directly the [SSOT](#ssot) and avoid an unnecessary conversion step.
To import references, it first double checks for the presence of duplicated [DOI](#doi)s and utilizes the [Crossref REST API](https://www.crossref.org/education/retrieve-metadata/rest-api) to retrieve metadata associated with the DOI, the support for other citation sources such as Europe PMC is in progress.
The structure-related fields are only subject to limited processing: basic formatting of the [MF](#mf) by subscripting of the numbers.
Due to limitations in Wikidata, the molecule names are dropped if they are longer than 250 characters and likewise the [InChI](#inchi) strings cannot be stored if they are longer than 1500 characters.

Uploaded taxonomical [DB](#db) identifiers are currently restricted to ITIS, GBIF, NCBI Taxon, Index Fungorum, IRMNG, WORMS, VASCAN, and iNaturalist, and newly [OTL](#otl).
The taxa levels are currently limited to family, subfamily, tribe, subtribe, genus, species, variety.
The importer checks for the existence of each item based on their [InChIKey](#inchikey) and upserts the compound with the *found in taxon* statement and the associated organisms and references.

###### [LNPN](#lnpn)

From the onset, [LNPN](#lnpn) has been importing data directly from the frozen tabular data of the LOTUS dataset ([https://doi.org/10.5281/zenodo.5794106](https://doi.org/10.5281/zenodo.5794106) [@doi:10.5281/zenodo.5794106]).
In future versions, [LNPN](#lnpn) will directly feed on the [SSOT](#ssot).
...
It will allow mass-editing of entries and navigate quick navigation in the [SSOT](#ssot) for the curation of new and existing entries.
...
In line with the overall LOTUS approach, any modification made in this curation interface will be mirrored after validation on Wikidata and [LNPN](#lnpn).
