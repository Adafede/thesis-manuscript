...
More detail about the data conversion method is available in the related [Material & Methods section](#mm-dc)
...
While the PDA intensity approximately follows the CAD intensity when a signal is present, some major molecules without a chromophore are ignored if PDA only is used.
The same complementarity can be observed between MS ionization modes (see light and dark blue traces).
...
While each CAD major peak has its corresponding peak in MS, the opposite does not hold true, as illustrated by the positive ionization mode at the end of the chromatogram.
...
The combination of the annotations of all three tools was then submitted to taxonomically informed metabolite annotation using TIMA v2.5.5 [@doi:10.5281/zenodo.6929297].
...
Having all features informed as previously described, guided data dimensionality reduction could be performed.
This step was necessary to further narrow down the amount of information to be finallly reported and analyzed.
...
The concept of peak purity could then be used  to evaluate the quality of the acquired fragmentation sppectrum [@doi:10.1021/acs.analchem.6b04358], or even try to deconvolute it [@doi:10.1021/ac400751j].

As previously described, while PI mode is usually preferred as it is expected to be more generic, the ionization efficiency (ratio to noise) is better in NI mode [@doi:10.1021/acs.analchem.7b00096].
...
The correspoonding Table for NI mode is avaiable in Appendix @sec:appendix-cascade-1. 

`//TODO`{.red}

As this filtering process was effective but not sufficient, an additional filter was implemented to further narrow down the number of features attributed to the major compounds.
...
This led to fewer features considered "major", as decribed in Table @tbl:cascade-3.
After all filtering steps, less than two features remained per CAD peak.

Table: Peak Shape Similarity and Taxonomy Filtered Features Statistics (Positive Ionization Mode) {#tbl:cascade-3}
...
The corresponding Table for NI mode is avaiable in Appendix @sec:appendix-cascade-2. 

`//TODO DEVELOP`{.red}

An alluvial plot showing the filtering process on the PI data is illustrated in @fig:cascade-5.
...
Polarity switching discussion same run pos neg