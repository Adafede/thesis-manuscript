...
Finally, different automated numerical and visual reports are generated as described in the [Illustration of the Results](#illustration-of-the-results).
...
To ensure the best possible alignment, this was also done on the other signals.
...
Very small, poorly defined peaks are not picked, while major peaks are, thus allowing for good peak-shape comparison with MS features later on.
...
The main advantage was it did not require any parameter, thus making it optimal for automation.
...
Sirius performed well in positive mode, but negative mode led to inconsistencies so MZmine3 was preferred for both modes (data not shown).
Peakonly was also used, but its integration with the rest of the workflow, needing finely defined isotope patterns, also led to worse performance.
Finally, some tests were performed using SLAW, but they did not reach the performance of Sirius and MZmine3.
...
The second step consisted of normalizing peaks' intensity together with a retention time normalization between boundaries.
The aim of this procedure was to improve peak shape similarity between the compounds ionizing poorly and the CAD trace.
Without this step, the MS features with the highest intensity would by default have a nicer peak shape and thus be more likely to be correlated to the CAD signal.
