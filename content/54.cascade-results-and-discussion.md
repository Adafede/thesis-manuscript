...
As a minimal input, the workflow requires [LC](#lc)-[MS/MS](#msms) data, supplemented by another detector (here [CAD](#cad) and PDA, as described in [Data Acquisition](#data-acquisition)).
...
UV spectra were acquired between 200 and 500nm and [MS](#ms) spectra between 100 and 1500*m/z*.
...
The chromatograms were encoded as `BasePeak_0` for the [BPI](#bpi) chromatogram, `PDA.1_TotalAbsorbance_0` for the UV chromatogram, and `UV.1_CAD_1_0` for the [CAD](#cad) chromatogram.
...
Peaks were then integrated using simple algorithmic as [CAD](#cad) signal contains only one dimension, thus not allowing for more elaborated techniques, such as the ones used in [MS](#ms) and PDA peak detection.
...
Sirius performed well in the PI mode, but the [NI](#ni) mode led to inconsistencies thus MZmine3 was preferred for both modes (data not shown).
...
The [IIN](#iin) was complemented using the [MS](#ms)^1^ adduct module of TIMA v2.5.5 [@doi:10.3389/fpls.2019.01329; @doi:10.5281/zenodo.6929297].
...
An example of a [CAD](#cad) peak containing multiple [MS](#ms)-features in both PI and [NI](#ni) modes is illustrated in Figure @fig:cascade-4.
...
Each panel is composed of PI and [NI](#ni) modes ([NI](#ni) mode shown as negative values). 
...
Overall, more features were detected in PI, compare to [NI](#ni) (3,610 vs 2,312).
Not only the total features were higher in PI, but also the number of features per [CAD](#cad) peak.
This is in line with previous conclusions stating that, while PI mode is usually preferred as it is expected to be more generic, the ionization efficiency (ratio to noise) is better in [NI](#ni) mode [@doi:10.1021/acs.analchem.7b00096].
A small difference of [CAD](#cad) detected peaks can be observed between the two acquisition. 
Polarity switching experiments with [MS](#ms) instruments reaching greater acquisition frequency, thus allowing to acquire PI and [NI](#ni) modes in a single run could avoid this difference.
...
|Peak ID|Peak RT [min]|Peak Area [%]|Feature ID|Feature RT [min]|Feature *m/z*|Structure|[InChIKey](#inchikey) 2D|Annotation Score|Chemical Pathway|Chemical Superclass|Chemical Class|Nearest Organism\footnotemark{}|Reference(s)|
|-:|-:|-:|-:|-:|-:|-----|------|--:|----|----|-----|--|-|
...
Of the 44 [CAD](#cad) peaks, 15 were associated with 2D structures previously reported in *S. chirayita*, and 31 in the genus *Swertia*.
...
The two tables correspond to the two views shown in Figure @fig:cascade-7, but this time with PI and [NI](#ni) merged.
...
This further reinforces the reason for using a [CAD](#cad) detector, as the PDA detector would not have detected these compounds, which represent the majority of the mass of the extract studied.
...
Figure @fig:cascade-9 illustrates the biggest cluster of the PI analysis of *S. chirayita*.
