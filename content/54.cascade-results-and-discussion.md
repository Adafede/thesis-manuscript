...
This further refines panel B, showing that some of the features annotated with the same molecular formula, C~30~H~48~O~3~, do not share the same structural annotation (MIJYXULNPSFWEK and WCGUUGGRBIKTOS).
...
Multiple annotations corresponded to 2D structures already reported in *S. chirayita*.

`//TODO DEVELOP`{.red}

`//TODO DEVELOP PEAK PURITY`{.red}

All together, the different relations between these features could also help reducing the complexity of the data, and are detailed in the next subchapter.

#### Dimensionality Reduction 

Having all features as previously described, guided data dimensionality reduction could occur.
...
`//TODO`{.red}
DEVELOP Ionization efficiency [@doi:10.1021/acs.analchem.7b00096]
