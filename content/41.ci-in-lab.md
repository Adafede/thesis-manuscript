...
This was because of the unequal repartition of contaminants and solvent carriers being present below 120 [*m/z*](#mz), as the ion 102.0346 in Figure @fig:ci-1.
...
The principle of using exclusion lists to expand coverage has already been demonstrated by iteratively excluding ions aready acquired [@doi:10.1007/s13361-017-1608-0].
...
While centrality is used since many years in social sciences [@doi:10/bx3m36], it is still seldomy if not used at all in [MN](#mn) analysis.
...
Improvements of the annotation tools used in the laboratory are described hereafter.
...
Only a small propotion of known [NP](#np)s have their [MS/MS](#msms) spectrum accessible, either in public databases or through the acquisition of standards internally.
The [ISDB](#isdb) compensates for the above mentioned, as it allows to cover a much larger proportion of [NP](#np)s.
The general process to annotate through an [ISDB](#isdb) consists in two steps.
The first step consits in calculating the spectra in silico.
The library developed to automate the construction of the [ISDB](#isdb) was called *Spectral Library Builder*.
The second step consits in comparing the experimental and in silico spectra and is performed with the *Spectral Library Matcher* library.
...
First, it made the spectral matching step available to all oerating systems, and not exclusively to Linux.
...
Among the new functionalities, the possibility to compute other spectral similarities than the classical cosine and modified cosine was added.
Despite known limitations, cosine-based scores remain the most used spectral similarity method [@doi:10.1371/journal.pcbi.1008724]. 
...
Currently recommended implemented similarity metrics are: `ModifiedCosine` , `MS2DeepScore` [@doi:10.1186/s13321-021-00558-4], and `Spec2Vec` [@doi:10.1371/journal.pcbi.1008724].

An overview of the performance of those similarity metrics on a benchmarking set of 22,251 features originating from <https:doi.org/10.5281/zenodo.5186176> [@doi:10.5281/zenodo.5186176] against the last version of the [ISDB](#isdb) is illustrated in Figure @fig:ci-5.
...
This was further reflected in distribution of the ranks.
...
As explained above, none of the methods alone provided satisfactory results alone (only half of the correct annotations are in the top five candidates at best, see Table @tbl:ci-2).
