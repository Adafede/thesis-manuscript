...
The first step consists in calculating the spectra in silico.
The library developed to automate the construction of the [ISDB](#isdb) was called *Spectral Library Builder*.
The second step consists in comparing the experimental and in silico spectra and is performed with the *Spectral Library Matcher* library.
...
While the `MS2DeepScore` method had the most desirable distribution for the correct annotations, it had a bad discriminating power, with a high proportion of incorrect annotations also scored well.
