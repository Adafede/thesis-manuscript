...
Within the above described process, the complex data are often mined to scrutinize the smallest quantitative change of a single or few metabolite(s).
...
In this classical view, the product of proteins are specialized metabolites.
...
Taking all these considerations into account, metabolomics, as other *-omics* requires advanced bioinformatics and chemoinformatics tools. 
...
As [MS](#ms) detectors and modes of acquisition were detailed before, the current section will focus on data generated by [MS](#ms)-based metabolomics and tools and resources required to extract biological knowledge out of them.
...
This step is crucial, because it then allows to exchange data between laboratories, and to process them with any software, open or not.
...
The isotopic pattern can be key to later determine the [MF](#mf) [@doi:10.1021/ed083p1761.2].
...
Then, according to a given simiarity threshod, *edges* are drawn between the nodes.
If two spectra share sufficient similarity, they will be linked with an edge.
...
These [MN](#mn) are also powerful visualization techniques [@doi:10.1021/acs.analchem.8b03099], which allow to further study certain spectra in their surrounding spectral context.

##### Data Contextualization (From Structures to Biological Context)

Once structurally annotated, a further contextualization of the features is possible.
